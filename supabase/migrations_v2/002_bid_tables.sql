...
-- ============================================================
-- TABLE 5: bids
-- Thin identity/status stub. Created at PDF upload time (frontend).
-- All extracted data (pricing, warranty, scope, etc.) lives in bid_scope.
-- This table is the FK anchor for 8+ child tables.
-- ============================================================
...
CREATE TABLE IF NOT EXISTS bids (
  ...
  request_id TEXT NOT NULL,                     -- Batch correlation, always generated by frontend
  storage_key TEXT,                              -- Stable Supabase Storage path (for idempotency)
  source_doc_url TEXT,                           -- Signed URL snapshot (convenience, not for idempotency)
  source_doc_mime TEXT DEFAULT 'application/pdf',

  -- Observability
  processing_attempts INTEGER DEFAULT 0,
  last_error TEXT,

  -- Display name (denormalized for sort/display)
  contractor_name TEXT NOT NULL DEFAULT 'TBD',

  -- User Actions (stay on bids — these are user interactions, not extraction data)
...
-- Idempotency: stable path-based (Supabase recommendation)
CREATE UNIQUE INDEX IF NOT EXISTS ux_bids_idem
  ON bids(project_id, COALESCE(request_id, ''), storage_key);

-- Partial unique on bid_index within project/request (Supabase recommendation)
CREATE UNIQUE INDEX IF NOT EXISTS ux_bids_project_request_bidindex
  ON bids(project_id, request_id, bid_index) WHERE bid_index IS NOT NULL;
...
-- ============================================================
-- TABLE 7: bid_scope
-- ALL extracted bid data lives here. 1:1 per bid.
-- Absorbs pricing, warranty, timeline, payment, extraction from bids.
-- Plus scope booleans, electrical work, accessories, line items.
-- Created/updated by MindPal callback via UPSERT ON CONFLICT (bid_id).
-- ============================================================
...
CREATE INDEX IF NOT EXISTS idx_bid_scope_amount ON bid_scope(total_bid_amount);
