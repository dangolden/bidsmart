/**
 * Generate SQL INSERT statements from CA Electrification Programs JSON.
 * Output: /tmp/ca-import-batch-N.sql files (one per batch of 10 programs)
 *
 * Usage:
 *   npx tsx scripts/import-incentives/generate-sql.ts
 */
...
ON CONFLICT (program_code) DO UPDATE SET
  ...
  const outPath = `/tmp/ca-import-batch-${batchNum}.sql`;
  writeFileSync(outPath, sql, 'utf-8');
  console.log(`Batch ${batchNum}: ${batch.length} programs → ${outPath}`);
}

console.log(`\nGenerated ${batchNum} batch files in /tmp/ca-import-batch-*.sql`);
