  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-6">
      <div className="flex items-start gap-4 mb-4">
        ...
        <div>
          <h2 className="text-lg font-semibold text-gray-900">Try the Tool</h2>
          <p className="text-sm text-gray-600 mt-1">
            Not ready to upload your own bids? Explore a demo comparison to see how it works.
          </p>
        </div>
      </div>
      ...
      <div className="space-y-3">
        {demos.map((demo) => (
          <button
            ...
            className="w-full bg-teal-50 hover:bg-teal-100 border border-teal-200 rounded-lg p-4 flex items-center gap-4 transition-colors text-left group"
          >
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-1">
                <h3 className="font-medium text-gray-900">{demo.project_name}</h3>
                <span className="text-xs px-2 py-0.5 rounded-full font-medium bg-teal-100 text-teal-700">
                  Demo
                </span>
              </div>
              <p className="text-sm text-gray-600 line-clamp-2">
                {demo.demo_description || 'See how BidSmart analyzes and compares real heat pump contractor bids'}
              </p>
              {demo.bidCount !== undefined && demo.bidCount > 0 && (
                <p className="text-xs text-gray-500 mt-1 flex items-center gap-1">
                  <FileText className="w-3 h-3" />
                  {demo.bidCount} {demo.bidCount === 1 ? 'bid' : 'bids'} included
                </p>
              )}
            </div>
